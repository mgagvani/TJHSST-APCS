...
class HashtableLinearProbe implements Hashtable
{
   ...
   {      
      ...
      else { // need to circle back
         i = 0;
         while(array[i] != null && i < index) {
            i++;
         }
      }
      return i;
   ...
   public boolean contains(Object obj)     
   {
      ...
      // otherwise search in linear probe manner
      //  int i = 0;
...
class HashtableRehash implements Hashtable
{
   ...
   public HashtableRehash(int size) //constructor
   {
      ...
      array = new Object[size];
      if(size % 2 == 1) constant = 2;
      else {
         for(int i = 1; i < (int)((size)/2); i+=2) {
            if(size % i != 0) {
               constant = i;
               break; 
            }
   ...
   public int rehash(int index)
   {
      ...
      while(array[index % array.length] != null && count > 0) {
         index += count;
         count--;
      }
      if(count < 0) return -1;
