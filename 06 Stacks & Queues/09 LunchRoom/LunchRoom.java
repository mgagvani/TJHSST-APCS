...
public class LunchRoom
{
   ...
   public static void lunchRoom(int TIME, PrintWriter outfile)
   {
      ...
      *************************/
      int[] numberOfCustomers = new int[4];
      ...
      int min = 0;
      while(min < TIME || pq.isEmpty() == false) {
         if(Math.random() < CHANCE_OF_CUSTOMER) {
            int gradYear = (int)(Math.random() * 4);

            pq.add(new Customer(min, gradYear + 2022));

            numberOfCustomers[gradYear]++;
            servingTime = timeToOrderAndBeServed();
         }

         if(servingTime == 0 && pq.isEmpty() == false) {
            int time = pq.peek().arrivalTime;
            Customer temp = pq.remove();
            int thisCustomersTime = min - time;
            if(thisCustomersTime > longestWait[temp.gradYear - 2022]) longestWait[temp.gradYear - 2022] = thisCustomersTime;
            totalWait[temp.gradYear - 2022] += thisCustomersTime;
         ...
         if(pq.size() > longestQueue) longestQueue = pq.size();
         min++;
      ...
      // PRINT
      // System.out.println(totalWait[0]/numberOfCustomers[0]);
      for(int i = 0; i < 4; i++) {
         if(i == 0) System.out.println("Senior\t\t\t" + numberOfCustomers[0] + "\t\t" + longestWait[0] + "\t\t" + (float)totalWait[0]/numberOfCustomers[0]);
         if(i == 1) System.out.println("Junior\t\t\t" + numberOfCustomers[1] + "\t\t" + longestWait[1] + "\t\t" + (float)totalWait[1]/numberOfCustomers[1]);
         if(i == 2) System.out.println("Sophomore\t\t" + numberOfCustomers[2] + "\t\t" + longestWait[2] + "\t\t" + (float)totalWait[2]/numberOfCustomers[2]);
         if(i == 3) System.out.println("Freshman\t\t" + numberOfCustomers[3] + "\t\t" + longestWait[3] + "\t\t" + (float)totalWait[3]/numberOfCustomers[3]);
      }
   ...
   public static class Customer implements Comparable<Customer>
   {
      ...
      public Customer(int arr, int gradyear) {
         this.arrivalTime = arr;
         this.gradYear = gradyear;
      }

      public int compareTo(Customer o) {
         if(gradYear < o.gradYear) {
            return -1;
         }
         else if(gradYear > o.gradYear) {
            return 1;
         }
         else {
            if(getArrivalTime() < o.getArrivalTime()) { // i came earlier
               return 1;
            }
      ...
      public String toString() {
         String gN = "";
         switch(gradYear) {
            case 2022: gN = "Se"; break;
            case 2023: gN = "Ju"; break;
            case 2024: gN = "So"; break;
            case 2025: gN = "Fr"; break;
         }

         return arrivalTime + ":" + gN;
