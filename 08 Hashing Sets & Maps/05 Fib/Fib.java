...
public class Fib
{
   public static final int FIBsubN = 43;
...
class Fib3 implements Fibber
{
   ...
   public Fib3()
   {
      if(myFibList.size() == 0) { // if not 0, assume already has values
         myFibList.add(0); // index 0
         myFibList.add(1); // index 1
         myFibList.add(1); // index 2
      }
   }
   public int fib(int n)
   {
...
class Fib4 implements Fibber
{
   ...
   {  //if the Map is null, . . .
      if(myFibMap == null) {
         ...
         myFibMap.put(1, 1); // don't have to start a 0, can start at 1 :)
         myFibMap.put(2, 1);
      }
   }
   public int fib(int n)
