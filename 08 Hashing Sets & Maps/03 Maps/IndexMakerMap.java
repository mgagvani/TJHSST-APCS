...
class DocumentIndex extends TreeMap<String, TreeSet<Integer>>
{
   ...
   */
   ...
   {
      String toRet = "";
      for(String k: this.keySet()) { // iterate over each key
         toRet += k + " "; // add the key to the string
         toRet += get(k).toString().substring(1, get(k).toString().length() - 1); // add each value to the string by using toString
         toRet += " \n"; // new line
      }
      return toRet;
   }
